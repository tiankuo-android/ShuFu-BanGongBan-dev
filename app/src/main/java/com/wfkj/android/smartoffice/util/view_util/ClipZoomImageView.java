...
//			once = false;
    ...
    public Bitmap clip() {
        ...
        draw(canvas);
        //
       return getCircleBitmap(Bitmap.createBitmap(bitmap, Math.abs(mHorizontalPadding),
                Math.abs(mVerticalPadding), Math.abs(getWidth() - 2 * mHorizontalPadding),
               Math.abs(getWidth() - 2 * mHorizontalPadding)));
    }
